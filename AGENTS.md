# AGENTS.md — AI-Native Knowledge Hub (`{{workspace-root}}`)

> File hướng dẫn cho coding agents (GPT Codex CLI và tương thích). Codex tự đọc `AGENTS.md` ở repo root.
> Tương đương `CLAUDE.md` (cho Claude Code). Hai file giữ cùng nội dung; sửa một, đồng bộ file kia.

## Workspace là gì

`{{workspace-root}}` là **knowledge hub điều phối AI** cho việc phát triển các hệ thống của bạn,
...
- **Markdown:** chỉ tạo ở `{{workspace-root}}\plans` hoặc `{{workspace-root}}\docs`, trừ khi user yêu cầu rõ (như file này).
...
- Reports: `{{workspace-root}}\plans\reports\`
- Plans: `{{workspace-root}}\plans\`
...
- Root files tự động theo tool: Claude Code dùng `CLAUDE.md`, Gemini CLI dùng `GEMINI.md`, GitHub Copilot dùng `.github/copilot-instructions.md`, GPT Codex dùng `AGENTS.md`.
