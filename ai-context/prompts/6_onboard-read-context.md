...
Dùng work-context path của project đó, KHÔNG phải `{{workspace-root}}`:

- `{{service-a}}`       → `path/to/your-repo-a` (skill phù hợp với domain của bạn)
- `{{service-b}}`       → `path/to/your-repo-b` (skill phù hợp với domain của bạn)
- `{{gateway-service}}` → `path/to/your-gateway` (skill phù hợp với domain của bạn)
- `{{core-system}}`     → skill phù hợp với domain của bạn
